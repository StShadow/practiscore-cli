...
Safe by default: it **previews and asks before changing anything**, **never emails shooters** unless you opt in, and **locks the match** once you've confirmed. Big runs (hundreds of shooters) show a progress bar and can be **resumed** if interrupted.
...
Exit codes: `0` success · `1` runtime error · `2` usage error (bad flags, no match selected, unreadable cookie file, a non-integer shooter ID, a broken `config.toml`) · `3` auth/session expired · `4` partial failure (`move` returned `taken`/blocked, or a bulk run had ≥1 failure) · `5` `move-bulk` stopped on throttling (resumable from checkpoint) · `130` declined the confirm prompt.

With `--json`, **stdout is nothing but JSON** — the progress bar, prompts, the `Audit:` path, and any error text go to stderr, so you can pipe stdout straight into `jq`.
...
- **`--dry-run` really changes nothing.** A dry run, and answering "no" at the prompt, both leave the match exactly as they found it — including the lock, which is only ever set *after* you confirm.
- **Locking:** once you confirm, the tool locks the match before making changes and **leaves it locked**, printing the lock status at the end. (Locking stops shooters from self-squadding on the website — it does **not** stop another *admin* working the same match at the same time.)
...
- **Audit log:** every attempted move — single `move` and `move-bulk` alike, successes *and* failures — is written to a local audit file (`~/.practiscore-squads/audit/<slug>.jsonl`) so you have a record. This file **contains shooter email addresses** — keep it local and don't share or commit it.
...
| Symptom | Fix |
|---|---|
| `session expired` (exit `3`) mid-run | Grab a fresh cookie (steps above), then **re-run the exact same command**. `move-bulk` picks up from its checkpoint — already-moved shooters are skipped, so nothing moves twice. |
